...
- 系统手动代理启用时会与界面内的代理端点比较；地址、端口不一致或填写无效时，顶部状态显示“未生效”，标题下的警告区并列展示系统实际代理和当前填写的代理，开关旁提示关闭后重开。编辑端点、窗口激活复读和切换开关后会同步更新警告。
...
- `LayoutControls()` 根据警告区是否显示排列输入卡片、开关和测试区；目标客户区约为 `340×410` 至 `520×490`，默认 `380×410`。
